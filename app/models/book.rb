...
class Book < ApplicationRecord
  ...
  class << self
    ...
    # 図書カードURLをscrapeしてファイルURLを取得する
    def card_to_file_url(card_url)
      match, author_id = card_url.match(/https?:\/\/www\.aozora\.gr\.jp\/cards\/(\d+)\/card\d+.html/).to_a
      return nil if !match
      ...
      match, file_id = href.match(/files\/(\d+_\d+)\.html/).to_a
      return nil if !match

      "https://www.aozora.gr.jp/cards/#{author_id}/files/#{file_id}.html"
    end
