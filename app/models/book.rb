...
class Book < ApplicationRecord
  ...
  def split_text(text, chars_per=800)
    # text = self.text
    text = text.gsub(/(\R{1,})/, '\1'+"\r\n") # 元からある改行を1つ増やす
                 .gsub(/\R/, "\r\n")  # 改行コードを\r\nに変える
                 .gsub(/(。)(?=[^（]*）|[^「]*」)/, "★★★") # （）と「」内の文末句点を★★★に一時変更
                 .gsub(/(。)/, '\1'+"\r\n") # その他の句点に改行追加
                 .gsub(/★★★/, "。") # ★を句点に戻す
                 .gsub(/([^一-龠々])（([\p{Hiragana}]*?)）/, '\1\2')  # ひらがなのあとに括弧が来る（＝外字が消えてる）場合、ふりがなの括弧を外して表示する
                 .gsub(/([一-龠々]+)（([\p{Hiragana}]*?)）/) {|string| # 漢字のあとに括弧が来る（＝ふりがな）場合、漢字部分が常用漢字ならそのまま、そうでなければふりがなを消す
                   words = $1.split("")
                   (words & USUAL_KANJIS).empty? ? string : words.join("")
                 }
